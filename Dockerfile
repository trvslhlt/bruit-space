...
# directory -- bruit-space depends on the sibling bruit-kit package via a
...
# dangling at build time as long as docker-compose's bind mount makes it
# real before anything actually imports from it. ---
FROM node:20-alpine AS dev
WORKDIR /workspace/bruit-space
COPY bruit-space/package*.json ./
RUN npm install
COPY bruit-space/ .
EXPOSE 5173
...
# dist/ genuinely exists at the path bruit-space's file: dependency
# resolves to. ---
...
COPY --from=build /workspace/bruit-space/dist /usr/share/nginx/html
EXPOSE 80
