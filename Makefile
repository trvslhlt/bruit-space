...
build-image:
	docker build --target runtime -f Dockerfile -t bruit-space ..

# Run the built production image locally (make build-image first).
run-image:
	docker run --rm -p 8080:80 bruit-space

# Playwright golden-path check against the running dev stack (`make up`
# first) -- see tests/verify.mjs for exactly what it covers.
verify:
